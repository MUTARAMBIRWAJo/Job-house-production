...
export default function EventsPage() {
    ...
    try {
      ...
      if (authUser) {
        ...
        if (profile?.role === 'artist' || profile?.role === 'admin') {
          await fetchEvents(profile.id, profile.role)
        }
      }
    } catch (error) {
  ...
  const fetchEvents = async (userId: string, role: string) => {
    try {
      const supabase = createClient()
                ...
                <div className="flex gap-4">
                  <div className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => setFormData(prev => ({ ...prev, needs_audio_coverage: checked }))}
                    />
                  ...
                  <div className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => setFormData(prev => ({ ...prev, needs_video_coverage: checked }))}
                    />
